...
[[_replication_no_replace]]
== Skipping Existing Keys

By default {project-title} overwrites keys on the target: a key that already exists on the target is replaced with the source value.

Use the `--no-replace` option to skip keys that are already present on the target instead of overwriting them.
Only keys that do not yet exist on the target are written; existing target keys are left untouched.

.No-replace replication example
[source,console]
----
include::{testdir}/replicate-no-replace[]
----

Existence is checked per key at write time using `EXISTS` on the target.
This is best-effort and not atomic against concurrent writes to the target (there is a small window between the check and the subsequent write), so it is intended for seeding a target that is not taking concurrent writes.

NOTE: `--no-replace` disables the <<_replication_compare,compare>> step, since skipped keys would otherwise be reported as mismatches.

[[_replication_ignore_expired]]
== Ignoring Source Expirations

During live replication {project-title} propagates removals: when a key is deleted or expires on the source, the corresponding key is removed on the target.

Propagating *expirations* is undesirable in some long-running scenarios — for example after a traffic cutover, when the target has become authoritative and is being written to independently, while the source is stale and its keys are merely expiring (TTL). Propagating those expirations would delete live keys on the target.

Use the `--ignore-expired` option to stop key *expirations* on the source from being propagated to the target. Keys that expire on the source (keyspace event `expired`) are left untouched on the target, while explicit deletions (`del` / `unlink`) still propagate as normal.

NOTE: `--ignore-expired` disables the <<_replication_compare,compare>> step, since keys retained on the target would otherwise be reported as mismatches.
