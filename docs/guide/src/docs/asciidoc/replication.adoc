...
[[_replication_no_delete]]
== Ignoring Source Deletions

During live replication {project-title} propagates deletions: when a key is deleted or expires on the source, the corresponding key is removed on the target.

In some long-running scenarios this is undesirable — for example after a traffic cutover, when the target has become authoritative and is being written to independently, while the source is stale and its keys are merely expiring. Propagating those expirations would delete live keys on the target.

Use the `--no-delete` option to stop deletions and expirations on the source from being propagated to the target. Keys removed or expired on the source are left untouched on the target; only live keys continue to be written.

NOTE: `--no-delete` disables the <<_replication_compare,compare>> step, since keys retained on the target would otherwise be reported as mismatches.
